...
public class TraitHelper implements Externalizable {
    ...
    public <T, K> T don( Activation activation, K core, Class<T> trait, Object initArgs, boolean logical, Mode... modes ) {
        if ( core instanceof Thing && ( (Thing) core ).getCore() != core ) {
    ...
    public void update( final FactHandle handle,
        ...
        ((NamedEntryPoint) h.getEntryPoint()).update( h,
                                                      ...
                                                      activation );
        if ( h.isTraitOrTraitable() ) {
            updateTraits( h, mask, modifiedClass, activation );
        }
    }
    ...
    public void insertLogical(final Activation activation,
        ...
        FactHandle handle = workingMemory.insertLogical( object, false );
